...
module.exports = async (client, options) => {
  ...
  const logChannel = (options && options.logChannel) || "logs"; // Default value: "AhtiSpam-logs". Explication: This is the channel where every report about spamming goes to. If it's not set up, it will attempt to create the channel.
 ...
 client.on("checkMessage", async (message) => {
  ...
  const MuteMember = async (m, muteMsg) => {
    for (var i = 0; i < messageLog.length; i++) {
      ...
      if(!ReportChannel){
        try{
            ReportChannel = await m.guild.channels.create('logs', {
              type: 'text',
              ...
              .then(m=> m.send(`Created **\`logs\`** channel since a channel for reports wasn't provided from beginning when setting up the module.`))
              .catch(console.error)
